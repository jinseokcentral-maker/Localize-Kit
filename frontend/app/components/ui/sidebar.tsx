...
const SIDEBAR_WIDTH_ICON = "3rem"; // 48px - 아이콘 버튼(32px) + 좌우 패딩(16px)
      ...
      <SidebarContext.Provider value={contextValue}>
        <TooltipProvider delayDuration={0}>
          <div
            ...
            className={cn(
              "group/sidebar-wrapper flex min-h-svh w-full has-[[data-collapsible=icon]]:sidebar-collapsible",
              className
            )}
            ref={ref}
            {...props}
    ...
    return (
      ...
      >
        <div
          ...
          className={cn(
            "duration-200 relative flex h-full flex-col bg-sidebar transition-[width] ease-linear",
            className
          )}
          {...props}
        >
...
>(({ className, onClick, ...props }, ref) => {
  ...
  return (
    <Button
      ...
      variant="ghost"
      size="icon-sm"
      className={cn("border-0 bg-transparent hover:bg-secondary", className)}
      onClick={(event) => {
        onClick?.(event);
        toggleSidebar();
      }}
...
const SidebarRail = React.forwardRef<
  ...
  React.ComponentProps<"div">
>((props, ref) => {
  return <div ref={ref} data-sidebar="rail" {...props} />;
});
...
>(({ className, ...props }, ref) => {
  return (
    <input
      ...
      data-sidebar="input"
      className={cn(
        "flex h-8 w-full rounded-md border border-sidebar-border bg-sidebar-accent px-3 py-1 text-sm text-sidebar-foreground ring-offset-sidebar transition-shadow",
        "file:border-0 file:bg-transparent file:text-sm file:font-medium file:text-sidebar-foreground placeholder:text-sidebar-foreground/50",
    ...
    if (typeof tooltip === "string") {
      return (
        <Tooltip>
          ...
          <TooltipContent side="right" align="start" hidden={!isCollapsed}>
            {tooltip}
          </TooltipContent>
        </Tooltip>
      );
  ...
  return (
    <Comp
      ...
      className={cn(
        ...
        "group-data-[collapsible=icon]:hidden",
        showOnHover &&
          "group-focus-within/menu-item:opacity-100 group-hover/menu-item:opacity-100 data-[state=open]:opacity-100 peer-data-[active=true]/menu-button:text-sidebar-foreground opacity-0",
        className
      )}
      {...props}
    />
...
>(({ asChild = false, size = "md", isActive, className, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      data-active={isActive}
      className={cn(
        "flex h-7 min-w-0 -translate-x-px items-center gap-2 overflow-hidden rounded-md px-2 text-sidebar-foreground outline-none ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0 [&>svg]:text-sidebar-foreground/50",
